...
abstract class ConfigTagParent {
  ...
  var sortMode = 0
  // 0 = sort by name, 1 = sort by value
  ...
  def containsTag(tagname: String) = this.getTag(tagname, false) != null
  ...
  def getTag(tagname: String): ConfigTag = this.getTag(tagname, true)

  def removeTag(tagname: String): Boolean = {
    val tag = this.getTag(tagname, false)
